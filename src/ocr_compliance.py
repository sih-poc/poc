...
def extract_text_from_image(image_path: str) -> str:
    """
    Extract text from an image using EasyOCR with confidence filtering.
    ...
    try:
        ...
        # Initialize EasyOCR reader (you can specify language(s), e.g., ['en', 'fr'])
        ...
        # Filter out low-confidence detections and clean the text
        confident_texts = [
            word[1].lower().strip() for word in result
            if float(word[2]) > 0.8 and word[1].strip()
        ]
